...
///
/// v3 runs HandlerScaffold + RouterAssembly + CrateManifest.
/// Generated output is now a self-contained Cargo crate that
/// `cargo build` directly — Cargo.toml + src/main.rs + handler
/// + router files all emitted in one pass.
...
fn stage_crate_manifest(plan: &CodegenPlan) -> Vec<GeneratedFile> {
    vec![
        GeneratedFile {
            ...
            contents: render_cargo_toml(&plan.crate_name),
        },
        GeneratedFile {
...
mod tests {
    ...
    fn generate_emits_self_contained_crate() {
        ...
        // 2 handlers + handlers/mod.rs + router.rs + lib.rs + Cargo.toml + main.rs
