//! `enter-challenge` — backend handler stub.
//!
//! Method: POST
...
/// `enter-challenge` request payload.
...
/// `enter-challenge` response payload.
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct Response {
...
/// router at `POST /challenges/:id/entries`.
///
...
pub async fn handle_post(_req: Request) -> Result<Response, anyhow::Error> {
    // TODO: implement enter-challenge (operator pays entry fee + opens upload slot).
    Ok(Response { ok: true })
}
...
mod tests {
    ...
    fn module_name_matches_key() {
        // Self-doc: this file lives at src/handlers/enter_challenge.rs
        // for backend key "enter-challenge".
        assert_eq!("enter_challenge", "enter_challenge");
    }
}
