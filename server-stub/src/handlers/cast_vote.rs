//! `cast-vote` — backend handler stub.
//!
//! Method: POST
...
/// router at `POST /entries/:id/vote`.
///
...
pub async fn handle_post(_req: Request) -> Result<Response, anyhow::Error> {
    // TODO: implement cast-vote (operator votes for one entry on a challenge).
    Ok(Response { ok: true })
}
...
mod tests {
    ...
    fn module_name_matches_key() {
        // Self-doc: this file lives at src/handlers/cast_vote.rs
        // for backend key "cast-vote".
        assert_eq!("cast_vote", "cast_vote");
    }
}
