//! `live-start` — backend handler stub.
//!
//! Method: POST
//! Path:   /live/start
...
pub async fn handle_post(_req: Request) -> Result<Response, anyhow::Error> {
    // TODO: implement live-start (user starts a live skill stream from the feed-top composer).
    Ok(Response { ok: true })
}
...
mod tests {
    ...
    fn module_name_matches_key() {
        // Self-doc: this file lives at src/handlers/live_start.rs
        // for backend key "live-start".
        assert_eq!("live_start", "live_start");
    }
}
