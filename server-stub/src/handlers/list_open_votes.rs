//! `list-open-votes` — backend handler stub.
//!
//! Method: GET
//! Path:   /votes
...
/// `list-open-votes` request payload.
...
/// `list-open-votes` response payload.
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct Response {
...
pub async fn handle_get(_req: Request) -> Result<Response, anyhow::Error> {
    // TODO: implement list-open-votes (open-vote queue (sidebar)).
    Ok(Response { ok: true })
}
...
mod tests {
    ...
    fn module_name_matches_key() {
        // Self-doc: this file lives at src/handlers/list_open_votes.rs
        // for backend key "list-open-votes".
        assert_eq!("list_open_votes", "list_open_votes");
    }
}
