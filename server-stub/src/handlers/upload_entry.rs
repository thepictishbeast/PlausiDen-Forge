//! `upload-entry` — backend handler stub.
//!
//! Method: POST
...
/// router at `POST /entries/:id/video`.
///
...
pub async fn handle_post(_req: Request) -> Result<Response, anyhow::Error> {
    // TODO: implement upload-entry (video upload pipeline (S3 multipart)).
    Ok(Response { ok: true })
}
...
mod tests {
    ...
    fn module_name_matches_key() {
        // Self-doc: this file lives at src/handlers/upload_entry.rs
        // for backend key "upload-entry".
        assert_eq!("upload_entry", "upload_entry");
    }
}
